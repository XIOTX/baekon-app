...
export default function BaekonAppContent({ userId }: BaekonAppContentProps) {
  ...
  const [workView, setWorkView] = useState<'box' | 'board'>('box');
  const [lifeView, setLifeView] = useState<'box' | 'board'>('box');
  ...
  useEffect(() => {
    // Initialize schedView from localStorage
    const savedDefaultView = localStorage.getItem('baekon-default-view');
    if (savedDefaultView === 'calendar' || savedDefaultView === 'planner') {
      setSchedView(savedDefaultView);
    }

    // Initialize hourEvents from localStorage
    const savedHourEvents = localStorage.getItem('baekon-hour-events');
    if (savedHourEvents) {
      try {
        setHourEvents(JSON.parse(savedHourEvents));
      } catch (e) {
        console.warn('Failed to parse saved hour events:', e);
      }
    }

    // Initialize dayEvents from localStorage
    const savedDayEvents = localStorage.getItem('baekon-day-events');
    if (savedDayEvents) {
      try {
        setDayEvents(JSON.parse(savedDayEvents));
      } catch (e) {
        console.warn('Failed to parse saved day events:', e);
      }
    }
