              ...
              >
                ...
                <div className="space-y-1">
                  {events.map((event, idx) => (
                    <input
                      key={`${dayKey}-event-${idx}-${event.slice(0, 10)}`}
                      type="text"
                      defaultValue={event}
    ...
    <div className="h-full p-3 flex flex-col overflow-hidden">
      ...
      <div className="mb-1 flex-shrink-0">
        ...
        <div className="flex space-x-1 mb-1">
          {weekDates.map((date, index) => (
            <div key={`day-${date.toISOString()}-${index}`} className="flex-1">
              <button
          ...
          <div className="neon-panel p-2 border-neon-blue relative z-10">
            <div className="flex space-x-2">
              {mondaysInMonth.map((monday, index) => (
                <button
                  key={`monday-${monday.toISOString()}-${index}`}
      ...
      <div className="flex-1 grid grid-cols-2 gap-2 min-h-0">
        {timeQuarters.map((quarter, quarterIndex) => (
          <div key={`quarter-${quarter.label}-${quarterIndex}`} className="neon-panel p-2 relative z-10 flex flex-col min-h-0">
            ...
            <div className="space-y-1 flex-1 overflow-y-auto">
              {quarter.hours.map((hour, hourIndex) => {
                ...
                return (
                  <div
                    key={`hour-${quarter.label}-${hour}-${hourIndex}`}
                  ...
                  >
                    <div className="flex items-start gap-2">
                      ...
                      <div className="flex-1">
                        ...
                        {events.map((event, idx) => (
                          <div key={`${hourKey}-display-${idx}-${event.slice(0, 10)}`} className="text-xs text-gray-300 font-red-hat mb-1">
                            • {event}
                          </div>
                        ))}
