...
export default function Sidebar({ selectedDate, setSelectedDate }: SidebarProps) {
  ...
  // Auto-scroll to current year when year selector opens
  useEffect(() => {
      ...
      <div className="neon-panel h-full p-3 relative z-10 overflow-y-auto">
        ...
        <div className="mb-4">
          ...
          {showYearSelector && (
            <div
              ref={yearScrollRef}
              className="absolute top-12 left-2 right-2 z-20 neon-panel max-h-48 overflow-y-auto scroll-smooth"
            >
              <div className="py-2">
                {generateYearRange().map(year => (
                  <button
                    ...
                    className={`w-full p-2 text-center hover:bg-neon-purple hover:bg-opacity-20 transition-all duration-200 ${
                      year === currentYear
        ...
        <div className="space-y-2">
          {months.map((month, index) => (
            ...
            >
              <div className="font-syne-mono text-sm tracking-wider">
                {month}
              </div>
            </button>
          ))}
